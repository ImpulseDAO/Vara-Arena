...
fn spending_all_gas_strategy() {
    ...
    let hash2: [u8; 32] = system
        .submit_code("../../target/wasm32-unknown-unknown/release/spending_all_gas.wasm")
        .into();
    let code_id2 = CodeId::from(hash2);
...
fn wrong_reply_format() {
    ...
    let hash2: [u8; 32] = system
        .submit_code("../../target/wasm32-unknown-unknown/release/wrong_reply_format.wasm")
        .into();
    let code_id2 = CodeId::from(hash2);
    ...
    let result = arena.send(USER_ID, ArenaAction::Play { lobby_id: 0 });
